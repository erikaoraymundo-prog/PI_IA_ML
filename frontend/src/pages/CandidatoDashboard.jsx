...
const CandidatoDashboard = ({ user }) => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Inicializar com dados da conta
        setProfileData(prev => ({
          ...prev,
  ...
  useEffect(() => {
    fetchCandidaturas();
    fetchProfile();
  }, [fetchCandidaturas, fetchProfile]);
  ...
  const handleCancelApplication = async (appId, jobTitle) => {
    ...
    try {
      ...
      showFeedback(`Candidatura para "${jobTitle}" cancelada com sucesso!`);
      fetchCandidaturas();
    } catch (err) {
      console.error("Erro ao cancelar candidatura:", err);
      showFeedback('Erro ao cancelar candidatura. Tente novamente.', 'error');
    } finally {
      setCancelingId(null);
    }
  };
  ...
  // Upload de arquivo de Currículo no Perfil
  ...
  // Executar candidatura automática retroativa (em vagas existentes)
  const runRetroactiveAutoApply = async () => {
    if (!user?.uid || !profileData.resumeText) return;
    try {
      // 1. Carregar vagas abertas
      ...
      // 2. Carregar candidaturas existentes do usuário para evitar duplicidade
      ...
      for (const match of highMatches) {
        ...
        // Congelar snapshot do perfil no momento da aplicação
        const snapshot = {
  ...
  const handleToggleAutoApply = async () => {
    ...
    if (!currentStatus) {
      ...
      if (!validateProfileForAutoApply()) {
        alert("Complete seu perfil para liberar a candidatura automática. Preencha Nome, E-mail, Telefone, Cidade/Estado e anexe seu currículo na aba Editar Perfil.");
        return;
      }
  ...
  const confirmActivateAutoApply = async () => {
    ...
    try {
      const docRef = doc(db, 'users', user.uid);
      // Salva o perfil completo primeiro
      const payload = {
        ...profileData,
      ...
      // Roda as candidaturas nas vagas existentes compatíveis
  ...
  return (
    <div className="container" style={{ padding: '4rem 1rem', minHeight: 'calc(100vh - 80px)' }}>
      ...
      <div className="agente-header" style={{
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: '600px' }}>
          ...
          <p style={{ fontSize: '1rem', opacity: 0.85, color: '#94a3b8', margin: 0 }}>
            Gerencie suas informações profissionais e acompanhe os matches automáticos da inteligência artificial.
          </p>
        </div>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: '0.75rem', opacity: 0.8, color: '#e2e8f0', marginTop: '0.2rem' }}>
              Aplicar em vagas compatíveis (≥ 70%)
            </div>
          </div>
        ...
        loading ? (
          ...
          <div className="card" style={{
            ...
            <p style={{ color: 'var(--text-muted)', maxWidth: '400px', margin: '0 auto 2rem' }}>
              Você ainda não se inscreveu em nenhuma vaga. Complete seu perfil profissional e ative a candidatura automática para darmos match!
            </p>
          </div>
        ) : (
          <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                      ...
                      {app.appliedAutomatically && (
                        <span style={{
                          ...
                          alignItems: 'center',
                          gap: '0.25rem'
                        }} title="Inscrição realizada automaticamente pela IA">
                          🤖 Automática
                        </span>
                      )}
                    </div>
      ...
      {/* CONTEÚDO TAB: EDITAR PERFIL (ACORDEÕES) */}
      {activeTab === 'perfil' && (
              ...
              <div className="acc-content">
                ...
                <div>
                  ...
                  <textarea 
                    required 
                    rows={4}
                    placeholder="Escreva um breve resumo destacando suas principais conquistas, stack técnica e objetivos profissionais..."
                    className="profile-input" 
                    ...
                    <div key={idx} className="dynamic-item-box">
                      ...
                      <div>
                        ...
                        <textarea 
                          required 
                          rows={3}
                          placeholder="Descreva suas responsabilidades, tecnologias utilizadas e resultados alcançados..."
                          className="profile-input" 
                ...
                {profileData.educations.length === 0 ? (
                  ...
                  profileData.educations.map((edu, idx) => (
                    <div key={idx} className="dynamic-item-box">
                      ...
                      <div className="profile-grid-2" style={{ marginTop: '0.5rem' }}>
                        <div>
                          <label className="profile-label">Nível de Formação *</label>
                          <select 
                            className="profile-input"
                            value={edu.level}
            ...
            {expandedSections.documentos && (
              <div className="acc-content">
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <label className="profile-label">Currículo Atualizado (Formatos aceitos: PDF, DOCX — Limite: 5MB) *</label>
        ...
        <div className="overlay" onClick={() => setShowAutoApplyConfirm(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ padding: '2rem', maxWidth: '460px' }}>
            ...
            <p style={{ fontSize: '0.9rem', color: '#64748b', lineHeight: 1.5, marginBottom: '1.5rem', textAlign: 'center' }}>
              Ao ativar, você autoriza o nosso sistema de IA a fazer o seu match e <strong>inscrever seu perfil automaticamente</strong> em qualquer vaga ativa ou futura do portal que possua compatibilidade <strong>igual ou superior a 70%</strong>. 
            </p>
