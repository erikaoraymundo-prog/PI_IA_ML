...
const VagasListaPage = lazy(() => import('./pages/VagasListaPage'));
...
function App() {
  ...
  const [regData, setRegData] = useState({ fullName: '', email: '', password: '', userType: 'candidato', cnpj: '' });
  ...
  const fetchJobs = async () => {
    try {
      ...
      setJobs(allJobs.filter(j => j.ativo));
    } catch (err) {
  ...
  const nextSlide = () => {
    if (jobs.length > itemsToShow) {
      setCarouselIndex((prev) => (prev + 1) % (jobs.length - itemsToShow + 1));
    }
  };

  const prevSlide = () => {
    if (jobs.length > itemsToShow) {
      setCarouselIndex((prev) => (prev - 1 + (jobs.length - itemsToShow + 1)) % (jobs.length - itemsToShow + 1));
    }
  };
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setRegData({ fullName: '', email: '', password: '', userType: 'candidato', cnpj: '' });
    } catch (err) {
  ...
  const handleCandidateClick = async () => {
    ...
    setLoading(true);
    try {
      // 1. Obter os dados atualizados do candidato
      const userDoc = await getDoc(doc(db, "users", user.uid));
      if (!userDoc.exists() || !userDoc.data()?.resumeText) {
        alert("Você ainda não cadastrou um currículo em seu perfil. Vamos redirecioná-lo para a página de edição de perfil para que possa anexar seu currículo.");
        setCurrentPage('editar_perfil');
        return;
      }
      ...
      const THRESHOLD = 8;
      const allScores = calculateMatchScores(resumeText, activeJobs);
      ...
      alert("Erro ao realizar match com seu perfil: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyToJobs = async () => {
    ...
    try {
      ...
      if (!downloadURL) {
        alert("Currículo não encontrado no seu perfil. Por favor, acesse o menu superior e clique em 'Editar Perfil' para anexar seu currículo.");
        setApplying(false);
        return;
      }
      ...
      for (const match of matches) {
        ...
        const snapshot = {
          ...
          modelPreferences: userData.modelPreferences || [],
    ...
    <div className="app-wrapper">
        ...
        <div className="container navbar-container">
          ...
          <div className={`nav-wrapper ${isMenuOpen ? 'open' : ''}`}>
            <div className="nav-links">
              ...
              <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('vagas'); setIsMenuOpen(false); }} className={currentPage === 'vagas' ? 'active' : ''}>Vagas</a>
              ...
              {user && (user.userType === 'admin' || isAdmin) ? (
                <>
                  <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('perfil_empresa'); setIsMenuOpen(false); }} className={`mobile-only-link ${currentPage === 'perfil_empresa' ? 'active' : ''}`} style={{ color: currentPage === 'perfil_empresa' ? '#00a896' : undefined }}>
                    🏢 Editar Dados Empresa
                  </a>
                  <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('editar_perfil'); setIsMenuOpen(false); }} className={`mobile-only-link ${currentPage === 'editar_perfil' ? 'active' : ''}`} style={{ color: currentPage === 'editar_perfil' ? '#00a896' : undefined }}>
                    👤 Editar Dados Candidato
                  </a>
                </>
              ) : (
                user && (
                  <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage(user.userType === 'empresa' ? 'perfil_empresa' : 'editar_perfil'); setIsMenuOpen(false); }} className={`mobile-only-link ${(currentPage === 'editar_perfil' || currentPage === 'perfil_empresa') ? 'active' : ''}`} style={{ color: (currentPage === 'editar_perfil' || currentPage === 'perfil_empresa') ? '#00a896' : undefined }}>
                    👤 Editar Dados
                  </a>
                )
              )}
            ...
            <div className="auth-group">
              {user ? (
                  ...
                  <div className="profile-menu-container">
                    ...
                    {showDropdown && (
                      <div className="profile-dropdown">
                        ...
                        {user && (user.userType === 'admin' || isAdmin) ? (
                          <>
                            <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('perfil_empresa'); setShowDropdown(false); setIsMenuOpen(false); }}>
                              🏢 Editar Dados Empresa
                            </a>
                            <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('editar_perfil'); setShowDropdown(false); setIsMenuOpen(false); }}>
                              👤 Editar Dados Candidato
                            </a>
                          </>
                        ) : (
                          user && (
                            <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage(user.userType === 'empresa' ? 'perfil_empresa' : 'editar_perfil'); setShowDropdown(false); setIsMenuOpen(false); }}>
                              👤 Editar Dados
                            </a>
                          )
                        )}
                ...
                <button onClick={() => { setShowLoginModal(true); setIsMenuOpen(false); }} className="btn btn-primary">Começar</button>
              )}
            </div>
          </div>
        </div>
      ...
      {currentPage === 'home' && (
        <>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                {(!user || user.userType !== 'empresa' || isAdmin) && (
                  <button className="btn btn-primary" onClick={handleCandidateClick}>
                    {loading ? 'Processando...' : 'Verificar vagas compatíveis'}
                  </button>
                )}
                <input
                  id="resumeUpload"
                ...
                />
                {(!user || user.userType === 'empresa' || isAdmin) && (
                  <button className="btn btn-outline" onClick={handleRecrutadorClick} style={{ background: '#e1eaec', border: 'none' }}>
                    Cadastrar Nova Vaga
                  </button>
                )}
              </div>
          ...
          <section className="container" style={{ padding: '6rem 0', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                <span className="section-tag">Oportunidades em destaque</span>
                <h2 className="section-title" style={{ margin: '0.5rem 0 0 0' }}>Vagas abertas agora</h2>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button 
                    ...
                    disabled={jobs.length <= itemsToShow}
                  >
                    &larr;
                  </button>
                  <button 
                ...
                <a href="#" onClick={(e) => { e.preventDefault(); setCurrentPage('vagas'); }} style={{ color: '#00a896', fontWeight: 600, textDecoration: 'none', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  Ver todas &rarr;
                </a>
              </div>
            </div>
            ...
            {jobs.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3rem', border: '1px solid #cbd5e1', borderRadius: '16px', background: '#f8fafc' }}>
                <p style={{ color: 'var(--text-muted)', margin: 0 }}>Nenhuma vaga ativa disponível no momento.</p>
              </div>
            ) : (
              <div className="carousel-outer" style={{ overflow: 'hidden', width: '100%', position: 'relative', padding: '0.5rem 0' }}>
                <div 
                ...
                >
                  {jobs.map((job) => (
                    ...
                    >
                      <div className="job-card" style={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: '340px' }}>
                        <div className="job-card-header" style={{ marginBottom: '1.25rem' }}>
                          ...
                          <span className="job-tag" style={{ textTransform: 'uppercase' }}>
                            {job.escala_trabalho || 'remoto'}
                          </span>
                        </div>
                        <h3 className="job-title" style={{ fontSize: '1.15rem', fontWeight: 800, color: '#1e293b', marginBottom: '0.4rem', lineHeight: '1.4' }}>{job.titulo}</h3>
                        <p className="job-meta" style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '1rem' }}>
                          🏢 {job.empresa_nome} • 📍 {job.localizacao}
                        </p>
                        
                        {job.requisitos_tecnicos && (
                          <div className="job-skills" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', marginBottom: '1.25rem' }}>
                            {job.requisitos_tecnicos.slice(0, 3).map((req, i) => (
                              <span key={i} className="skill-badge" style={{ background: '#f8fafc', color: '#64748b', fontSize: '0.75rem', padding: '0.2rem 0.5rem', borderRadius: '6px', border: '1px solid #e2e8f0', fontWeight: 500 }}>
                                {req}
                              </span>
                            ))}
                          </div>
                        ...
                        <p style={{ fontSize: '0.85rem', color: '#475569', lineHeight: '1.5', marginBottom: '1.5rem', display: '-webkit-box', WebkitLineClamp: '3', WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {job.descricao}
                        </p>
                        ...
                        <button 
                          ...
                          className="btn btn-outline" 
                          style={{ marginTop: 'auto', width: '100%', padding: '0.6rem', borderColor: '#cbd5e1' }}
                        >
      ...
      {currentPage === 'vagas' && (
        <Suspense fallback={
          ...
          <VagasListaPage user={user} onLoginRequired={() => setShowLoginModal(true)} />
        </Suspense>
      )}
      {currentPage === 'admin' && (
        <Suspense fallback={
      ...
      {currentPage === 'editar_perfil' && (
        <Suspense fallback={
          <div className="container" style={{ padding: '8rem 2rem', textAlign: 'center', minHeight: '80vh' }}>
            <div style={{ width: '50px', height: '50px', border: '4px solid #f3f3f3', borderTop: '4px solid #00a896', borderRadius: '50%', margin: '0 auto 1.5rem', animation: 'spin 1s linear infinite' }}></div>
            <p style={{ color: 'var(--text-muted)' }}>Carregando editor de perfil...</p>
            <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
          </div>
        }>
      ...
      {currentPage === 'perfil_empresa' && (
        <Suspense fallback={
          <div className="container" style={{ padding: '8rem 2rem', textAlign: 'center', minHeight: '80vh' }}>
            ...
            <p style={{ color: 'var(--text-muted)' }}>Carregando perfil da empresa...</p>
            <style>{`@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }`}</style>
          </div>
        }>
